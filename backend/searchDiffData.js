  ...
  030303001,ការទិញផ្ទះផ្សេងទៀត,Other House Purchase,030303,ការទិញផ្ទះផ្សេងទៀត,Other House Purchase,0303,ការអោយខ្ចីទិញផ្ទៈជាកម្មសិទ្ធិផ្ទាល់ខ្លួន,"Mortgages, Owner-Occupied Housing only",03,តម្រូវការផ្ផាល់ខ្លួន,Personal Essentials
    ...
    function createTableRows() {
        ...
        for (let item of rowsAsObjects2) {
            ...
            tr.appendChild(td3);
            let td7 = createImageCell('assets/images/dropdown.png', rowId, 'subRow' + rowId, 'mainRow' + rowId, item["2 Name (EN)"], item['2 Name (KH)']);
            tr.appendChild(td7);
            ...
            for (let subitem of subitems) {
                ...
                let tdSubImage = createImageCell2('assets/images/dropdown.png', rowId, 'subSubRow' + rowId + '_' + subRowId, 'subRow' + rowId + '_' + subRowId, subitem['4 Name (EN)'], subitem['4 Name (KH)']); 
                ...
                for (let subitem2 of subitems2) {
                    ...
                    let tdSubSubImage = createImageCell3('assets/images/dropdown.png', rowId, 'subSubSubRow' + rowId + '_' + subRowId + '_' + subSubRowId, 'subSubRow' + rowId + '_' + subRowId + '_' + subSubRowId, subitem2['6 Name (EN)'], subitem2['6 Name (KH)']); 
                    ...
                    for (let subitem3 of subitems3) {
                        ...
                        let tdSubSubSubImage = createImageCell4('assets/images/copy-icon.webp', rowId, 'subSubSubRow' + rowId + "_" + subRowId + "_" + subSubRowId + "_" + subSubSubRowId, subitem3, subitem3['9 Name (EN)'], subitem3['9 Name (KH)']);
    ...
    function storeId(rowId){
        ...
        let prefix;
        if (rowId.includes("_")) {
            prefix = rowId.substring(0, rowId.lastIndexOf("_")); // Extracts the pattern before the last underscore
            let activeIds = document.querySelectorAll('[id^="' + prefix + '"]');
            for(let x of activeIds){
                activeId.push(x);
            }
        } else {
            prefix = rowId.split(/(\d+)/)[0]; 
            let activeIds = document.querySelectorAll('[id^="' + prefix + '"]');
            for(let x of activeIds){
                activeId.push(x);
            }// Extracts the non-digit prefix from the rowId;
        }
    }
    ...
    function showNavigationBar(rowId) {
        let navigationBar = document.getElementById('navigation-bar');
        navigationBar.textContent += ' > ' + rowId;
    }    
    function firstNavigationBar(rowId){
        let navigationBar = document.getElementById('navigation-bar');
        let navBar = document.getElementById('navigation-bar-kh');
        navigationBar.textContent = '';
        navBar.textContent = '';
        navigationBar.textContent += rowId
    }
    function firstNavigationBarKh(rowId){
        let navigationBar = document.getElementById('navigation-bar-kh');
        navigationBar.textContent += rowId
    }
    function showNavigationBarKh(rowId) {
        let navigationBar = document.getElementById('navigation-bar-kh');
        navigationBar.textContent += ' > ' + rowId;
    }    
    function createImageCell(imageUrl, rowId, controlledRowId, activeCellId, navigation, navigationKh) {
        let td = document.createElement('td');
        ...
        btn.onmousedown = function() { 
            ...
            firstNavigationBar(navigation);
            firstNavigationBarKh(navigationKh);
    ...
    function createImageCell2(imageUrl, rowId, controlledRowId, activeCellId, navigation, navigationKh) {
        let td = document.createElement('td');
        td.style.textAlign = 'center'; // Center the content
        let btn = document.createElement('button');
        btn.style.border = 'none';
        ...
        btn.onmousedown = function() { 
            ...
            showNavigationBarKh(navigationKh);
            hideRows(controlledRowId);
            let tableHead = document.querySelector('#dataTable thead tr'); // Select the table header row
            let headers = tableHead.querySelectorAll('th'); // Get all the headers
            // Loop through each header and change it
    ...
    function createImageCell3(imageUrl, rowId, controlledRowId, activeCellId, navigation, navigationKh) {
        let td = document.createElement('td');
        td.style.textAlign = 'center'; // Center the content
        let btn = document.createElement('button');
        btn.style.border = 'none';
    ...
    function createImageCell4(imageUrl, rowId, controlledRowId, itemToString, navigation, navigationKh) {
        let td = document.createElement('td');
        td.style.textAlign = 'center'; // Center the content
        let btn = document.createElement('button');
        btn.style.border = 'none';
